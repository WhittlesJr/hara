...
(defn args-classify
  ...
  (cond (sort-terms arg)                 :sort-terms
        ...
        (or (string? arg) (check/regexp? arg))  :name
        (fn? arg)                        :predicate
        (set? arg)                       :origins
        (and (vector? arg)
...
(defn args-convert
  ...
  {:added "3.0"}
  [args]
  (let [single-fn #(or (class/class-convert %) %)]
    (mapv (fn [v]
            (cond (vector? v)
                  (mapv single-fn v)
                  ...
                  (symbol? v)
                  (or (primitive/primitive-type v :class)
                      v)
                  :else v))
          args)))
